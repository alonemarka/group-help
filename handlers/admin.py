...
def is_admin_or_owner(update: Update) -> bool:
    ...
    # Owner ise direkt izin ver
    if user_id in OWNER_IDS:
        return True
    
    # Grup admini mi kontrol et
    try:
...
async def mute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin_or_owner(update):
        ...
        return
    if not update.message.reply_to_message: 
        await update.message.reply_text("⚠️ Bir mesaja reply yapın.")
        return
    user = update.message.reply_to_message.from_user
...
async def unmute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🔊 {user.mention_html()} susturulması açıldı.", parse_mode='HTML')
